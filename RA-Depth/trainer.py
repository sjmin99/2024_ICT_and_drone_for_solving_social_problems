...
class Trainer:
    def __init__(self, options):
        ...
        ##########test phase#######
        valid_path = os.path.join(os.path.dirname(__file__), "splits", 'eigen', "{}_files.txt")
        val_test_filenames = readlines(valid_path.format("test"))
        ...
        train_dataset = self.dataset(
            self.opt, self.opt.data_path, train_filenames, self.opt.height, self.opt.width,
            self.opt.frame_ids, 4, is_train=True, img_ext=img_ext)
        self.train_loader = DataLoader(
            train_dataset, self.opt.batch_size, True,
            num_workers=self.opt.num_workers, pin_memory=True, drop_last=True)
        val_dataset = self.dataset(
            self.opt, self.opt.data_path, val_filenames, self.opt.height, self.opt.width,
        ...
        self.val_test_dataset = self.dataset(self.opt, self.opt.data_path, val_test_filenames,
                                           self.opt.height, self.opt.width,
                                           [0], 4, is_train=False, img_ext=img_ext)

        self.val_test_loader = DataLoader(
            self.val_test_dataset, self.opt.batch_size, False,
            num_workers=self.opt.num_workers, pin_memory=True, drop_last=False)

        self.val_iter = iter(self.val_test_loader)

        self.num_val_test_samples = len(val_test_filenames)
    ...
    def run_epoch(self):
        ...
        for batch_idx, inputs in enumerate(self.train_loader):
            ...
            avg_photometric_loss += losses["loss_MiS"]
            ...
            if early_phase or late_phase:
                ...
                if not self.opt.debug:
                    visuals = OrderedDict([('input_LoS_image', tensor2im2(inputs[('color_LoS',0,0)])),
                            ('input_MiS_image', tensor2im2(inputs[('color_MiS',0,0)])),
                            ('input_HiS_image', tensor2im2(inputs[('color_HiS',0,0)])),
                            ('LoS_disp', tensor2im2(outputs['out_LoS'][('disp',0)])),
                            ('MiS_disp', tensor2im2(outputs['out_MiS'][('disp',0)])),
                            ('HiS_disp', tensor2im2(outputs['out_HiS'][('disp',0)])),
                            ])

                    for name in losses.keys():
                        wandb.log({'train_'+name:losses[name]})
        ...
        if not self.opt.debug:
            avg_photometric_loss /= len(self.train_loader)
            # avg_supervised_loss /= len(self.train_loader)
            wandb.log({'avg_photometric_loss':avg_photometric_loss})
            # wandb.log({'train_supervised_loss':avg_supervised_loss})

        self.set_eval()

        self.calculate_depth_loss(val_test=True)
        # self.val()
    ...
    def predict_poses(self, inputs, features, type='M'):
        ...
        for scale in self.opt.scales:
            ...
            for i, frame_id in enumerate(self.opt.frame_ids[1:]):
                ...
                pix_coords_LoS = self.project_3d[source_scale](
                    cam_points_LoS, inputs[("K_LoS", source_scale)], T.detach(), inputs[("dxy_LoS")])
    ...
    def compute_losses(self, inputs, outputs):
        ...
        total_loss = (loss_MiS + loss_LoS) + 1.0*(loss_consis_LoS)
    ...
    def calculate_depth_loss(self,val_test):
        ...
        if val_test :
            dataloader = self.val_test_loader
            num_samples = self.num_val_test_samples
            flag = 'test_' 
        else:
            # dataloader = self.val_train_loader
            # num_samples = self.num_val_train_samples 
            flag = 'train_'
        ...
        with torch.no_grad():
            ...
            for i, data in tqdm(enumerate(dataloader)):
                ...
                #--------------------------------------------------------------------------------
                #output = disparity map dict, outputs = model dict
                
                features = self.models["encoder"](input_color) #Encoding Features
...
#             outputs["out_MiS"] = self.models["depth"](features_MiS)
                ...
                # gt_depths = data['depth_gt'].squeeze(1).cpu().numpy()
        ...
        mean_errors = (np.array(errors).sum(0) / num_samples)
        metric_dict = {}
        ...
        if not self.opt.debug:
            for name, value in zip(metric_names, mean_errors):
                wandb.log({'val_'+flag+name : value})
